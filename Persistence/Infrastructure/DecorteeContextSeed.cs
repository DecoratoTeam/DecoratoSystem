        ...
        public static async Task SeedAsync(DecorteeDbContext context)
        {
            if (!await context.Users.AnyAsync())
            {
                ...
                var seedUser = new User
                {
                    Name = "Seed User",
                    UserName = "seeduser",
                    Email = "seed@decorato.com",
                    Role = Role.Customer
                };
                seedUser.Password = passwordHasher.HashPassword(seedUser, "Seed123!");

                await context.Users.AddAsync(seedUser);
                await context.SaveChangesAsync();
            }

            var seedUserId = await context.Users.Select(u => u.Id).FirstAsync();
            ...
            if (!await context.ShowcaseDesigns.AnyAsync())
            {
                ...
                var roomNamesById = await context.RoomTypes.ToDictionaryAsync(r => r.Id, r => r.Name);
                ...
                for (int i = 1; i <= 500; i++)
                {
                    ...
                    var roomName = roomNamesById[roomId];
